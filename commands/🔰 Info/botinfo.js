...
module.exports = {
  ...
  run: async (client, message, args, prefix) => {
    try {
      ...
      if (!bot || bot == null || bot.id == null || !bot.id) return message.reply("Bot no encontrado");
      ...
      embedbotinfo.setAuthor("Información:   " + bot.username + "#" + bot.discriminator, bot.displayAvatarURL({ dynamic: true }))
      embedbotinfo.addField('**❱ botname:**', `<@${bot.username}>\n\`${bot.tag}\``, true)
      embedbotinfo.addField('**❱ ID:**', `\`${bot.id}\``, true)
      embedbotinfo.addField('**❱ Avatar:**', `[\`Enlace al avatar\`](${bot.displayAvatarURL({ format: "png" })})`, true)
      embedbotinfo.addField('**❱ Fecha de ingreso a DC:**', "\`" + moment(bot.createdTimestamp).format("DD/MM/YYYY") + "\`\n" + "`" + moment(bot.createdTimestamp).format("hh:mm:ss") + "\`", true)
      embedbotinfo.addField('**❱ Es un robot:**', `\`${bot.bot ? "✔️" : "❌"}\``, true)
