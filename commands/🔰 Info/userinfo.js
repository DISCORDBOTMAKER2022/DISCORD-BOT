...
module.exports = {
  ...
  run: async (client, message, args, prefix) => {
    try {
      ...
      embeduserinfo.setAuthor("Información:   " + member.user.username + "#" + member.user.discriminator, member.user.displayAvatarURL({ dynamic: true }), "https://discord.gg/FQGXbypRf8")
      ...
      embeduserinfo.addField('**❱ Fecha de ingreso a DC:**', "\`" + moment(member.user.createdTimestamp).format("DD/MM/YYYY") + "\`\n" + "`" + moment(member.user.createdTimestamp).format("hh:mm:ss") + "\`", true)
      embeduserinfo.addField('**❱ Fecha de ingreso:**', "\`" + moment(member.joinedTimestamp).format("DD/MM/YYYY") + "\`\n" + "`" + moment(member.joinedTimestamp).format("hh:mm:ss") + "\`", true)
      embeduserinfo.addField('**❱ Banderas:**', `\`${userFlags.length ? userFlags.map(flag => flags[flag]).join(', ') : 'None'}\``, true)
      embeduserinfo.addField('**❱ Estado:**', `\`${statuses[member.user.presence.status]} ${member.user.presence.status}\``, true)
      embeduserinfo.addField('**❱ Rol más alto:**', `${member.roles.highest.id === message.guild.id ? 'None' : member.roles.highest}`, true)
      embeduserinfo.addField('**❱ es un robot:**', `\`${member.user.bot ? "✔️" : "❌"}\``, true)
      var userstatus = "Not having an activity";
      if (activity) {
        if (activity.type === "CUSTOM_STATUS") {
          let emoji = `${activity.emoji ? activity.emoji.id ? `<${activity.emoji.animated ? "a" : ""}:${activity.emoji.name}:${activity.emoji.id}>` : activity.emoji.name : ""}`
          userstatus = `${emoji} \`${activity.state || 'No tener actividad.'}\``
        }
        else {
