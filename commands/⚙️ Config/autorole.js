  ...
  run: async (client, message, args, prefix) => {
    ...
    if (message.mentions.roles.first()) {
      ...
      if (data) {
        ...
        message.channel.send(`Autorole está activo y el rol establecido en ${message.mentions.roles.first()}`);
      ...
      if (data2) {
        ...
        return message.channel.send(`Autorole ha sido desactivado!`);

      } else if (!data2) {
        return message.channel.send(`Autorole no está configurado!`)
      }
    }
  }
}