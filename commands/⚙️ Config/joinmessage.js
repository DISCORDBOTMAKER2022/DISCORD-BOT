  ...
  run: async (client, message, args, prefix) => {
    ...
    if (text !== "off") {
      ...
      if (data) {
        ...
        newData.save();
        message.channel.send(`Unirse Mensaje establecido en ${newData.JoinMsg}`);
      } else if (!data) {
        let newData = new Joinmsg({
          JoinMsg: args.join(" "),
          GuildID: message.guild.id,
      ...
      if (data2) {
        ...
        return message.channel.send(`Unirse Mensaje ha sido desactivado!`);
      } else if (!data2) {
        return message.channel.send(`¡El mensaje de unión no está configurado!`);
      }
    }
  }
}